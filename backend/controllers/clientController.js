...
//   const { name, user_id, app_code, api_secret } = req.body;
//   if (!name || !user_id || !app_code || !api_secret) {
//     return res.status(400).json({ success: false, message: 'All fields are required' });
//   }

//   const [result] = await db.query(
//     'INSERT INTO clients (name, user_id, app_code, api_secret) VALUES (?, ?, ?, ?)',
//     [name, user_id, app_code, api_secret]
//   );

//   res.json({ success: true, message: 'Client added successfully', data: { id: result.insertId } });
// };
...
// // GET login URL for a client
// exports.getLoginUrl = async (req, res) => {
//   const [rows] = await db.query('SELECT app_code FROM clients WHERE id = ?', [req.params.id]);
//   if (!rows.length) return res.status(404).json({ success: false, message: 'Client not found' });
...
// // POST - Exchange authCode for session (called after user logs in through Alice Blue)
...
//   const client = rows[0];
//   const checksum = aliceBlueService.createChecksum(client.user_id, authCode, client.api_secret);

//   const sessionData = await aliceBlueService.getUserSession(checksum);

//   if (sessionData.stat !== 'Ok') {
//     return res.status(400).json({ success: false, message: sessionData.emsg });
//   }
...
// // GET client profile from Alice Blue
...
//     return res.status(401).json({ success: false, message: 'Client session not activated. Please login first.' });
//   }
...
// // GET client limits/funds
// exports.getClientLimits = async (req, res) => {
//   const [rows] = await db.query('SELECT * FROM clients WHERE id = ?', [req.params.id]);
//   if (!rows.length) return res.status(404).json({ success: false, message: 'Client not found' });

//   if (!rows[0].user_session) {
//     return res.status(401).json({ success: false, message: 'Client not logged in' });
//   }
...
// // GET all client sessions status
// exports.getSessionStatus = async (req, res) => {
//   const [clients] = await db.query(
//     `SELECT id, name, user_id, is_active,
//      CASE WHEN user_session IS NOT NULL AND session_expires_at > NOW() THEN 'active' ELSE 'inactive' END as session_status,
//      session_expires_at
